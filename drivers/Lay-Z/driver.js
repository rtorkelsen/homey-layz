  ...
  async loginHandler(data) {
    ...
    for (const server of servers) {
      ...
      try {
        ...
        const { token, uid, expire_at } = response.data;
  
        // Save token, uid, and expire_at into Homey Manager Settings
        await this.homey.settings.set('token', token);
        await this.homey.settings.set('uid', uid);
        await this.homey.settings.set('expire_at', expire_at);
        await this.homey.settings.set('baseUrl', server.baseUrl);
        await this.homey.settings.set('appId', server.appId);
